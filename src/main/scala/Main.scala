...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val tweetsWithSentimentSchema = StructType(
      Array(
        StructField("status_id", StringType),
        StructField("user_id", StringType),
        StructField("created_at", DateType, nullable = true),
    ...
    val sentimentPath = "s3n://spark-project-test/twitter-data/Sentiment_Data_Eng.csv"
    ...
    val tweetsWithSentimentRDD =
      if (fs.exists(new Path(sentimentPath)))
        // read data with sentiment
        spark.read.schema(tweetsWithSentimentSchema).option("header", "true").csv(sentimentPath).as[TweetWithSentiment].rdd
        ...
        val tweetsDF = spark.read
          ...
          .where(
            col("text").isNotNull and // remove nulls
              ...
              col("lang") === "en" and // take only english tweets
    ...
    println("Clean data rows: " + tweetsWithSentimentRDD.count())
    tweetsWithSentimentRDD.toDF.show()
    ...
    val tweetsPerDay = tweetsWithSentimentRDD.map(tweet => (tweet.created_at, (1, tweet.sentiment)))
    ...
    val mostPopularTweets = tweetsWithSentimentRDD.map(row => MostPopularTweets(row.status_id, row.screen_name, row.text, row.retweet_count, row.favourites_count))
    ...
    val mostPopularUsers = tweetsWithSentimentRDD.map(row => (row.screen_name, (row.retweet_count, row.favourites_count)))
    ...
    val mostActiveUsers = tweetsWithSentimentRDD.map(tweet => (tweet.screen_name, 1))
      .reduceByKey(_ + _)
    ...
    val wordsRDD = tweetsWithSentimentRDD.map(_.text.toLowerCase())
      .flatMap(_.split(" "))
    ...
    val hashtagOfTheDayRDD = tweetsWithSentimentRDD.map(tweet => (tweet.created_at, tweet.text))
    ...
    val sortedByAvgSentiment = tweetsWithSentimentRDD.map(row => (row.screen_name, 1))
      .reduceByKey(_ + _)
      .join(tweetsWithSentimentRDD
    ...
    val influentialSenstimentsRDD = tweetsWithSentimentRDD
